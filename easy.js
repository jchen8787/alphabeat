...
function keepTime() {
	...
	beatOffset = elapsed%774; //774ms per beat
...
var render = function() {
	...
	//start of new beat
    if (lastOffset >=674 && beatOffset<=100) {
		determineCurrentLetter();
		drawBeatLetter()
		dropLetter(curLetter);
	}

    if (lastOffset<387 && beatOffset>=387) { //half way through the beat
...
function handleKeyDown(event) { 
    if(event.keyCode >= 65 && event.keyCode <= 90 && !beatLock) { //a-z
		...
		//232ms margin of error
		if ((beatOffset<232||beatOffset>542)&&(acceptedLindex==inputLindex))
			greenBool = true;
		else redBool = true;
